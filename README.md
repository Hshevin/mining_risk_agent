...
> **介绍**：基于 Harness 工程化管控的工矿企业风险预警 LLM 智能体系统，支持 Stacking 集成学习预测、GLM-5 决策智能体 Workflow、三重校验风控拦截、模型自动迭代 CI/CD；前后端独立部署 —— FastAPI 后端 + React/Vite SCADA 风格 SPA 前端，`docker compose up --build` 一键启动。
...
| 低 | `*_1.*` 历史备份 | 已从仓库移除（保留 `venv/` 内第三方包文件不动） |
