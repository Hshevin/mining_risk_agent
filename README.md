...
> **介绍**：基于 Harness 工程化管控的工矿企业风险预警 LLM 智能体系统，支持 Stacking 集成学习预测、GLM-5 决策智能体 Workflow、三重校验风控拦截、模型自动迭代 CI/CD。代码采用 **Python monorepo 三包架构**（`mining_risk_common` / `mining_risk_serve` / `mining_risk_train`）+ 独立 React 前端，`docker compose up --build` 一键启动。
...
```bash
export MINING_PROJECT_ROOT="$(pwd)"   # 项目根目录，供 config.yaml / datasets / artifacts 解析

# 后端 API 运行时（等价于 Docker 默认 requirements-deploy 的 serve 层）
uv pip install -r requirements-serve.txt

# 安装 monorepo 三包（可编辑模式）
uv pip install -e packages/mining_risk_common \
               -e packages/mining_risk_train \
               -e packages/mining_risk_serve

# 完整能力：训练 + RAG/NLP + 测试工具
uv pip install -r requirements-full.txt
uv pip install -e "packages/mining_risk_train[ml]"   # TensorFlow / SHAP 等可选 ML 栈
```

| 依赖文件 | 用途 |
|----------|------|
| `requirements-common.txt` | 共享包 `mining_risk_common` |
| `requirements-serve.txt` | 后端 API + Agent |
| `requirements-train.txt` | 训练脚本与迭代流水线 |
| `requirements-deploy.txt` | Docker API 镜像（serve + train + 树模型推理） |
| `requirements-full.txt` | 本地研发全量依赖 |

### 4. 准备数据

将 `公开数据(1).zip` 解压后，将「数据补充 / 数据参考 / 新数据 / 企业相关表导出 / 事故数据*.xlsx」放入 `datasets/raw/public/` 对应子目录，预合并宽表 `new_已清洗.xlsx` 放入 `datasets/interim/merged/`（详见 `datasets/README.md`，可在 `config.yaml` 中修改 `data.raw_data_path`）。
...
训练完成后，模型与 Pipeline 默认写入 `artifacts/models/`、`artifacts/pipelines/`（路径由 `config.yaml` 的 `model.stacking.model_path` / `pipeline_path` 控制）。

#### 训练 NER 实体抽取模型

```bash
python scripts/train_ner.py --data datasets/processed/ner_train.json --output artifacts/models/ner_model.pt --epochs 10
```
...
python -c "from mining_risk_serve.iteration.monitor import ModelMonitor; ModelMonitor().should_retrain()"

# 或通过 API 触发（需 X-Admin-Token）
curl -X POST http://localhost:8000/api/v1/iteration/trigger -H "X-Admin-Token: $MRA_ADMIN_TOKEN"
```
...
| 服务 | 镜像 | 容器名 | 端口映射 | 说明 |
|------|------|--------|----------|------|
| api | `mining-risk-agent-api` | `mining_risk_api` | `127.0.0.1:8000:8000` | `mining_risk_serve` + 三包 editable 安装；默认 `requirements-deploy.txt` |
| frontend | `mining-risk-agent-frontend` | `mining_risk_frontend` | `8501:80` | React/Vite SPA + Nginx 反代 `/api`、`/health`、`/docs` |

开发时 `docker-compose.yml` 将 `./packages` 挂载到容器 `/app/packages`，修改 Python 源码后重启 `api` 服务即可生效（无需重建镜像）。
...
```python
import asyncio
from mining_risk_serve.harness.memory import LongTermMemory
...
```python
from mining_risk_serve.harness.nlp_pipeline import NERPipeline
...
```python
from mining_risk_serve.harness.reranker import Reranker
...
**网络爬虫**（`packages/mining_risk_common/src/mining_risk_common/dataplane/crawler.py`）：
...
```python
from mining_risk_serve.harness.validation import compliance_checker, logic_checker, feasibility_checker, run_march_validation
...
```python
from mining_risk_serve.harness.monte_carlo import SamplingNode
...
```python
from mining_risk_serve.harness.validation import ToolCallInterceptor
...
```python
from mining_risk_serve.agent.workflow import DecisionWorkflow, ScenarioConfig
...
| 阶段 | 模块（包） | 关键动作 |
|------|------------|---------|
| **监控触发** | `mining_risk_serve.iteration.monitor` | 新增样本 >5000 或近期 F1<0.85 触发 `should_retrain()` |
| **自动训练** | `mining_risk_train.iteration.pipeline` | 数据清洗 → 特征工程 → 时序 CV → 候选模型序列化至 `artifacts/models/` |
| **Git Flow** | `mining_risk_serve.iteration.gitflow` | 从 `main` 创建 `feature/model_v{x}`，生成 PR 模板，配置分支保护 |
| **回归测试** | `mining_risk_train.iteration.regression_test` | 同源测试集对比：准确率/精确率/召回率/F1/AUC + SHAP Kendall Tau 稳定性 |
| **Drift 分析** | `mining_risk_train.iteration.drift_analysis` | 基学习器权重差异、元学习器系数变化、Pipeline 步骤变更检测 |
| **两级终审** | `mining_risk_serve.iteration.approval_fsm` | `PENDING_REVIEW → SECURITY_APPROVED → TECH_APPROVED → STAGING → PRODUCTION → ARCHIVED` |
| **预生产监控** | `mining_risk_serve.iteration.staging_monitor` | 每 5 分钟采样：延迟 P99、异常率、置信度分布、输入漂移；24h 后自动出报告 |
| **灰度发布** | `mining_risk_serve.iteration.canary` | 流量比例阶梯切换：`0.0 → 0.1 → 0.5 → 1.0` |
...
# result → {"model_version": "v2", "model_path": "artifacts/models/stacking_risk_v2.pkl", ...}
```
...
```python
from mining_risk_serve.iteration.approval_fsm import ApprovalFSM
...
- **单元测试**：pytest 全量测试
- **回归测试**：自动调用 `mining_risk_train.iteration.regression_test` 对比新旧模型
...
```
                    ┌─────────────────┐
                    │    frontend/    │  React SPA（仅 TypeScript）
                    └────────┬────────┘
                             │ HTTP /api/v1
                    ┌────────▼────────┐
                    │ mining_risk_serve│  FastAPI、Agent、Harness、LLM
                    │  api / agent /   │  运行时 iteration（审批、灰度、演示）
                    │  harness / llm   │
                    └────────┬────────┘
              ┌──────────────┼──────────────┐
              │              │              │
     ┌────────▼────────┐    │    ┌─────────▼─────────┐
     │mining_risk_common │◄───┘    │ mining_risk_train │
     │ dataplane / model │         │ train / pipeline  │
     │ utils / demo      │         │ regression / drift│
     └───────────────────┘         └───────────────────┘
```

| Python 包 | 导入前缀 | 职责 |
|-----------|----------|------|
| `mining_risk_common` | `mining_risk_common.*` | 数据加载与特征工程、Stacking 模型核心、全局配置、演示 Mock |
| `mining_risk_serve` | `mining_risk_serve.*` | HTTP API、LangGraph 决策工作流、Harness（RAG/记忆/校验）、运行时迭代 |
| `mining_risk_train` | `mining_risk_train.*` | 离线训练、`TrainingPipeline`、回归/漂移测试、可视化 |
| `mining_risk_compat`（过渡期） | `mining_risk.*` | 旧路径重导出，导入时触发 `DeprecationWarning` |

**依赖关系**：`serve → common + train`，`train → common`（无反向依赖）。仓库根目录的 `datasets/`、`artifacts/`、`config.yaml`、`var/` 由 `MINING_PROJECT_ROOT` 或向上查找 `config.yaml` 定位，不随包安装路径变化。

### 12.2 后端分层架构（`mining_risk_serve`）

```
HTTP 请求
    ↓
Router（mining_risk_serve/api/routers/*）     ← HTTP 绑定、参数校验、依赖注入
    ↓ Depends
Service（mining_risk_serve/api/services/*）   ← 业务编排、异常转换、Mock 降级
    ↓ 调用
Interface（mining_risk_serve/api/interfaces/*） ← Protocol 抽象，便于测试替换
    ↓ 实现
Harness / Agent / Train 包                   ← 领域能力与模型训练
```

| 层级 | 路径 | 职责 |
|------|------|------|
| 契约层 | `mining_risk_serve/api/schemas/` | 统一请求/响应 DTO（Pydantic） |
| 接口层 | `mining_risk_serve/api/interfaces/` | `RiskPredictor`、`FeaturePipeline` 等 Protocol |
| 服务层 | `mining_risk_serve/api/services/` | `PredictionService`、`ResourceRegistry` |
| 路由层 | `mining_risk_serve/api/routers/` | 瘦路由 + `Depends` 注入 |
| 异常处理 | `mining_risk_serve/api/exception_handlers.py` | 统一 `ApiResponse` 错误信封 |

**依赖注入示例：**

```python
# mining_risk_serve/api/routers/prediction.py
...
**资源注册表（`mining_risk_serve/api/services/dependencies.py`）：**
...
后端 `mining_risk_serve/api/schemas/common.py` 与前端 `frontend/src/api/types/common.ts` 对齐：
...
| 模块 | 文件 | 主要模型 |
|------|------|----------|
| 通用 | `mining_risk_serve/api/schemas/common.py` | `ApiResponse`、`ErrorDetail`、`HealthPayload` |
| 预测/决策 | `mining_risk_serve/api/schemas/prediction.py` | `PredictRequest`、`DecisionRequest`、`DecisionResponse` |
| 知识库 | `mining_risk_serve/api/schemas/knowledge.py` | `KnowledgeUpdateRequest`、`KnowledgeMutationResponse` |
| 数据 | `mining_risk_serve/api/schemas/data.py` | `DataUploadResponse`、`BatchUploadRequest` |
| 审计 | `mining_risk_serve/api/schemas/audit.py` | `AuditLogRequest`、`AuditLogEntry` |

#### Protocol 抽象接口

`mining_risk_serve/api/interfaces/prediction.py` 定义以下 Protocol：
...
| `frontend.demo_data` | `mining_risk_common.demo` |

### 12.7 后续演进建议

| 优先级 | 模块 | 建议 |
|--------|------|------|
| 高 | `mining_risk_serve/api/routers/memory.py` | 拆分为 `MemoryService`，消除 Router 内业务逻辑 |
| 高 | `mining_risk_serve/api/routers/iteration.py` | 抽取 `IterationService` |
| 中 | 契约 | 从 `/openapi.json` 生成前端 TypeScript 类型 |
| 低 | `mining_risk_compat` | 过渡期结束后移除旧 `mining_risk.*` 重导出 |

### 12.8 项目目录结构

```
mining_risk_agent/
├── packages/             # Python monorepo（三包 + 兼容层）
│   ├── mining_risk_common/   # 共享：dataplane、stacking、utils、demo
│   ├── mining_risk_serve/    # 后端：api、agent、harness、llm、运行时 iteration
│   ├── mining_risk_train/    # 训练：train、visualization、训练 iteration
│   └── mining_risk_compat/   # 弃用：mining_risk.* 重导出（过渡期）
├── frontend/                    # React/Vite SPA（仅 TypeScript，见下方展开）
│   ├── src/api/                 # http.ts + client.ts + types
...
│   └── processed/
├── var/                  # 运行时（Git 忽略）
├── artifacts/            # 模型 .pkl 与 pipeline .pkl
├── scripts/                     # train_model.py, init_knowledge_base.py …
├── tests/                       # pytest（conftest 注入 packages/*/src）
├── prompts/                     # 场景化 Jinja2 模板
├── reports/figures/
├── knowledge_base/
├── memory/
├── config.yaml
...
1. **后端 Mock**：`mining_risk_serve/api/services/prediction_service.py` 调用 `mining_risk_common.demo.generate_mock_decision()`（HTTP 200 + `mock=true`）
2. **前端本地 Mock**：`frontend/src/data/demoData.ts` 提供与后端格式一致的兜底 JSON；`/api` 不可达时由 `RiskPredictionPage` 直接渲染
...
npm install
npm run dev
# http://localhost:5173 — /api 失败时自动使用 demoData.ts 本地 Mock
```
...
| 本地 Mock 兜底 | 后端离线降级 | `src/data/demoData.ts` 与 `mining_risk_common.demo` 三场景 Mock 对齐；API 失败时前端直接渲染 |
| Nginx 反向代理 | 同源避免跨域 | 前端容器内 Nginx 把 `/api`、`/health`、`/docs` 转发到 `api:8000`，并关闭 `proxy_buffering` 以保留 SSE 流 |

### 13.6 演示数据字段说明

`mining_risk_common/demo/data.py`（后端 Mock）与 `frontend/src/data/demoData.ts`（前端 Mock）中预置的 3 组高危企业数据字段保持一致；每组均包含以下核心字段：
...
```bash
export MINING_PROJECT_ROOT="$(pwd)"
export GLM5_API_KEY=test-key   # 配置校验占位
...
# 全部测试（排除重模型 / 可选依赖用例）
pytest tests/ -v \
  --ignore=tests/test_nlp_pipeline.py \
  --ignore=tests/test_crawler.py
```

### Demo Replay 数据源（模型迭代路演）

当前模型迭代系统默认使用 `DemoReplayDataSource`，从 `datasets/demo/*.json` 读取可回放演示批次。每个批次都包含 `metadata.batch_id`、`sample_count`、`risk_sample_count`、`recent_f1` 和 `description`，后端会基于这些元信息和批次内的门禁结果生成真实接口响应、回放报告和可追踪记录，而不是把触发逻辑写死在前端。
