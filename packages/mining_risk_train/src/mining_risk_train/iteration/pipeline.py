...
from mining_risk_train.train import evaluate_model, load_and_merge_data, prepare_features, split_data
...
class TrainingPipeline:
    ...
    def __init__(
        ...
        default_models = resolve_project_path(
            Path(config.paths.artifacts_root) / "models"
        )
        self.models_dir = str(models_dir or default_models)
        self.config = config
    ...
    def run(
        ...
        default_model_path = str(resolve_project_path(self.config.model.stacking.model_path))
        default_pipeline_path = str(resolve_project_path(self.config.model.stacking.pipeline_path))
        try:
            import shutil
