...
class DataLoader:
    ...
    def merge_enterprise_tables(
        ...
        Args:
            tables: 表名字典
            join_keys: 关联主键列表，默认尝试 ["企业ID", "统一社会信用代码", "主键ID"]
        ...
        if join_keys is None:
            join_keys = ["企业ID", "统一社会信用代码", "主键ID", "生产经营地址id"]
        
        # 找到存在的主
        first_df = list(tables.values())[0]
        actual_key = None
        for key in join_keys:
            if key in first_df.columns:
                actual_key = key
                break
        ...
        for name, df in tables.items():
            ...
            if merged is None:
                ...
                # 避免列名冲突，添加前缀
                overlap_cols = [c for c in df.columns if c in merged.columns and c != actual_key]
                rename_map = {c: f"{name}_{c}" for c in overlap_cols}
                df_renamed = df.rename(columns=rename_map)
                merged = pd.merge(merged, df_renamed, on=actual_key, how="outer")
        
        logger.info(f"多表合并完成，关联键: {actual_key}, 最终形状: {merged.shape}")
        return merged
